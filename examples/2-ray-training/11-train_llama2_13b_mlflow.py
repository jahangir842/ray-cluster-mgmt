...
MLFLOW_EXPERIMENT   = "llama2-13b-fsdp"
...
def init_model() -> torch.nn.Module:
    ...
    Memory breakdown:
        Parameters:      13,015,864,320
        Model bf16:      ~26 GB
        Adam states:     ~52 GB  (2× model in bf16)
        Activations:     ~4-8 GB per GPU (seq_len=256, batch=1)
        Total training:  ~100-120 GB → requires FSDP across all 8 GPUs
        Per GPU (FSDP8): ~13-15 GB — fits in 24GB ✓
    """
...
def shard_model(model: torch.nn.Module, cpu_offload: bool = True):
    ...
    # LLaMA-2-13B has 40 transformer layers (vs 32 in LLaMA-3.1-8B)
...
if __name__ == "__main__":
    ...
    experiment_name        = f"llama2_13b_fsdp_{uuid.uuid4().hex[:8]}"
    ...
    print(f"Starting LLaMA-2-13B training | Ray experiment: {experiment_name}")
    print()
